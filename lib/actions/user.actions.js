...
export const fetchSpacesByUser = async(userID, pageNumber = 1, pageSize = 5) =>{
  try {
    ...
    const spaces = await User.findOne({_id:userID},{spaces:1})
                            .populate({path:'spaces', model: Space, select: 'space_name space_description space_image space_admin', populate:({path:'space_admin', select:'alias'}), limit: pageSize, skip: pageSize * (pageNumber-1)})
