...
export const fetchThreadById = async (id, limitSize=0) =>{
    try {
        await connectToDB()
        ...
        const totalComments = await Thread.findOne({_id : id})
                                        .populate('thread_comments')
        const hasMore = totalComments.thread_comments.length > limitSize + 5
        const totalCommentsLength = totalComments.thread_comments.length
        return {thread, hasMore, totalCommentsLength}
    } catch (error) {
