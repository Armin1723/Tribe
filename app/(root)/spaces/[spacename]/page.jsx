...
const page = async ({ params, searchParams }) => {
  ...
  let threads = [];
  if (result?.isMember)
    threads = await fetchThreadsBySpacename(
      params.spacename.split("-").join(" "),
  ...
  return (
    <div className="custom-scrollbar lg:mb-2 flex flex-col gap-2 justify-center items-center md:w-[67vw] max-lg:min-w-[85vw] max-sm:px-4">
      ...
      {result.isMember && (
        <Link
          href={`/spaces/${params.spacename}/create`}
          shallow
          className="fixed flex items-center justify-center overflow-hidden lg:bottom-10 bottom-20 right-5 lg:right-[23vw] h-12 z-10 aspect-square rounded-full bg-gradient-to-br from-blue-800/40 to-blue-500/80 hover:opacity-75 shadow-md cursor-pointer"
        >
          <Image
            src="/assets/create-thread.svg"
            alt="create thread"
            width={48}
            height={48}
        ...
        {result?.isMember ? (
          ...
          <div className="flex flex-col items-center justify-center gap-4 my-4 max-sm:text-sm italic">
            <h1 className="text-xl font-bold">
              Join the space to view threads
            </h1>
          </div>
        )}
      </div>
