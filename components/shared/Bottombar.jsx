    ...
    return(
        <section className="fixed flex gap-2 bottom-0 w-full md:hidden items-center justify-between px-6 py-2 bg-gradient-to-t from-[#1b1b1b] to-gray-900 rounded-t-md">
            {sidebarLinks.map((link =>{
                ...
                return(
                    <Link href={link.route} shallow key={link.label} onClick={playClick} className={`${isActive && 'bg-gradient-to-br from-blue-800/40 to-blue-500/80'} ${isCreateBlog && 'font-extrabold scale-125'} flex flex-col items-center justify-center p-2 rounded-3xl sm:rounded-lg sm:px-4 sm:py-1 sm:hover:opacity-75`}>
